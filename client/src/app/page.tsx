  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentMessage.trim()) {
      ...
      try {
        ...
        // to run on render
        let url = `https://perplexity-latest-jta8.onrender.com/chat_stream/${encodeURIComponent(userInput)}`;
        // to run locally 
        ...
        eventSource.onmessage = (event) => {
          try {
            ...
            // Attempt to "fix" invalid escape sequences (patch for bad \')
            raw = raw.replace(/\\'/g, "'");           // bad \' -> '
            raw = raw.replace(/""}/g, '"');          // remove double end quote
            raw = raw.replace(/""/g, '"');            // remove extra embedded quotes
